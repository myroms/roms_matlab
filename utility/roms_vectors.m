...
%
% ROMS_VECTORS:  Proccess ROMS vector data.
%
% [Uout,Vout]=roms_vectors(Uinp,Vinp,angle,umask,vmask,boundary)
%
% This function processes  ROMS vector data for either the full
% grid or boundary edges. The strategy is to get any horizontal
% vector field (Uinp,Vinp) at  RHO-points  for the event that a
% rotation to ROMS curvilinear grid is needed.  Then,  they are
% computed at the appropriate Arakawa C-grid location.
%
% On Input:
%
...
if (DoBoundary),
  ...
  for ib=1:4,
    if (boundary(ib)),
      if (~ufields(ib)),
        error([ 'ROMS_VECTORS - cannot find field: Uinp.',char(Bnames(ib))]);
      end,
      if (~vfields(ib)),
        error([ 'ROMS_VECTORS - cannot find field: Vinp.',char(Bnames(ib))]);
      end,
    end,
  end,
else,
    ...
    for ib=1:4,
      if (boundary(ib)),
        switch ib
          case {1}
            if (is2d),
              Uout.west=squeeze(0.5.*(Uinp.west(1,:)+Uinp.west(2,:)));
              Vout.west=squeeze(0.5.*(Vinp.west(1,1:Mr-1)+Vinp.west(1,2:Mr)));
          ...
          case {2}
            if (is2d),
              Uout.east=squeeze(0.5.*(Uinp.east(1,:)+Uinp.east(2,:)));
              Vout.east=squeeze(0.5.*(Vinp.east(1,1:Mr-1)+Vinp.east(1,2:Mr)));
              ...
              Uout.east=squeeze(0.5.*(Uinp.east(1,:,:)+Uinp.east(2,:,:)));
              Vout.east=squeeze(0.5.*(Vinp.east(2,1:Mr-1,:)+Vinp.east(2,2:Mr,:)));
          ...
          case {3}
            if (is2d),
              Uout.south=squeeze(0.5.*(Uinp.south(1:Lr-1,1)+Uinp.south(2:Lr,1)));
              Vout.south=squeeze(0.5.*(Vinp.south(:,1)+Vinp.south(:,2)));
          ...
          case {4}
            if (is2d),
              Uout.north=squeeze(0.5.*(Uinp.north(1:Lr-1,2)+Uinp.north(2:Lr,2)));
              Vout.north=squeeze(0.5.*(Vinp.north(:,1)+Vinp.north(:,2)));
