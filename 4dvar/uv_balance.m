...
% Compute surface baroclinic pressure gradient in XI-direction
% (at U-points).
%
...
gradPx_bar(1:L,1:M)=0.25.*(K.pm(1:L ,1:M)  +K.pm(2:Lp,1:M)).*            ...
                          (K.Hz(1:L ,1:M,N)+K.Hz(2:Lp,1:M,N)).*          ...
                          phix(1:L,1:M).*                                ...
                          K.umask(1:L,1:M);

% Compute interior baroclinic pressure gradient in the XI-direction
...
for k=Nm:-1:1,
  ...
  gradPx_bar(1:L,1:M)=gradPx_bar(1:L,1:M)+                               ...
                      0.25.*(K.pm(1:L ,1:M)  +K.pm(2:Lp,1:M)).*          ...
                            (K.Hz(1:L ,1:M,k)+K.Hz(2:Lp,1:M,k)).*        ...
                            phix(1:L,1:M).*                              ...
                            K.umask(1:L,1:M);

end,

% Compute surface baroclinic pressure gradient in ETA-direction
% (at V-points).
%
...
gradPy_bar(1:L,1:M)=0.25.*(K.pn(1:L,1:M )  +K.pn(1:L,2:Mp)).*            ...
                          (K.Hz(1:L,1:M ,N)+K.Hz(1:L,2:Mp,N)).*          ...
                          phie(1:L,1:M).*                                ...
                          K.vmask(1:L,1:M);
...
for k=Nm:-1:1,
  ...
  gradPy_bar(1:L,1:M)=gradPy_bar(1:L,1:M)+                               ...
                      0.25.*(K.pn(1:L,1:M )  +K.pn(1:L,2:Mp)).*          ...
                            (K.Hz(1:L,1:M ,k)+K.Hz(1:L,2:Mp,k)).*        ...
                            phie(1:L,1:M).*                              ...
                            K.vmask(1:L,1:M);

end,

clear gamma phie phix
...
%---------------------------------------------------------------------------
% Compute RHS term for balance free-surface elliptic equation.
%---------------------------------------------------------------------------
%
...
GradPx(2:Lm,2:M)=gradPx_bar(2:Lm,2:M);

[GradPx]=lateral_obc(GradPx,2,Lm,2,M,K.EW_periodic,K.NS_periodic);


GradPy(1:Lp,1:M)=0.0;                            % at V-points

GradPy(2:L,2:Mm)=gradPy_bar(2:L,2:Mm);

[GradPy]=lateral_obc(GradPy,2,L,2,Mm,K.EW_periodic,K.NS_periodic);

% Computer RHS term (m4/s2), at RHO-points.

zeta_rhs(2:L,2:M)=-K.pm(2:L,2:M).*(GradPx(2:L ,1:Mm )-                   ...
                                   GradPx(1:Lm,1:Mm ))-                  ...
                   K.pn(2:L,2:M).*(GradPy(1:Lm,2:M  )-                   ...
                                   GradPy(1:Lm,1:Mm)).*                ...
                  K.rmask(2:L,2:M);

[zeta_rhs]=lateral_obc(zeta_rhs,2,L,2,M,K.EW_periodic,K.NS_periodic);
