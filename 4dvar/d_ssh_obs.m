...
%  AVISO dataset. USERS can use this as a prototype for their application.
%
...
start_day = datenum(2004,1,1);
end_day   = datenum(2004,2,1);

D=load_ssh_data(GRDfile,start_day,end_day);
...
%  Set observation error covariance (squared units). Use 2 cm for
%  altimetry.

obs.error=ones(size(obs.value)).*0.0004;

%  Initialize global variance per state variables.

obs.variance=zeros([1 Nstate]);

obs.variance(state.zeta)=0.0004;
