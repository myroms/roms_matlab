...
%    GreatCircle  Swith indicating how to compute the grid distance:
...
if (ischar(G)),
  spherical=nc_read(G,spherical);
  if (ischar(spherical)),
    if (spherical == 'T' || spherical == 'F'),
      spherical=true;
    else
      spherical=false;
    end
  ...
  if (GreatCircle && spherical),
    Xr=nc_read(G,'lon_rho');
    Yr=nc_read(G,'lat_rho');
    Xu=nc_read(G,'lon_u');
    Yu=nc_read(G,'lat_u');
    Xv=nc_read(G,'lon_v');
    Yv=nc_read(G,'lat_v');
