function svan = sw_svan2(S,T,P,PR)
...
%----------------------
if nargin ~=4
   error('sw_svan2.m: Must pass 4 parameters')
end %if
...
% BEGIN
% -----
svan = (  ones(size(S)) ./ sw_pden(S,T,P,PR)) - ...
         (ones(size(S)) ./ sw_pden(35*ones(size(S)),zeros(size(S)),P,PR) );
