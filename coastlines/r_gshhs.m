...
if (contains(Fname,'gshhs_f'))         % Full resolution database
  C.lon=NaN+zeros(492283,1);
  C.lat=NaN+zeros(492283,1);
  C.area=zeros(41520,1);
  k=ones(41521,1);
  decfac=12500;
elseif (contains(Fname,'gshhs_h'))     % High resolution database
  C.lon=NaN+zeros(492283,1);
  C.lat=NaN+zeros(492283,1);
  C.area=zeros(41520,1);
  k=ones(41521,1);
  decfac=2500;
elseif (contains(Fname,'gshhs_i'))     % Intermediate resolution database
  C.lon=NaN+zeros(492283,1);
  C.lat=NaN+zeros(492283,1);
  C.area=zeros(41520,1);
  k=ones(41521,1);
  decfac=500;
elseif (contains(Fname,'gshhs_l'))     % Low resolution database
  C.lon=NaN+zeros(101023,1);
  C.lat=NaN+zeros(101023,1);
  C.area=zeros(10768,1);
  k=ones(10769,1);
  decfac=100;
elseif (contains(Fname,'gshhs_c'))     % Crude resolution databse
...
%  Here we try to reduce the number of points.
    ...
    xflag=0;
    if (max(x) > 180)   % First, save original curve for later if we
      sx=x;             % anticipatea 180-problem.
      sy=y;         
      xflag=1;
    end
